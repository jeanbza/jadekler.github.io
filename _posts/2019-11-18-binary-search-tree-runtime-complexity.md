...
- Q: What is the [base](https://en.wikipedia.org/wiki/Logarithm) of the log?
  ...
  A: Typically when we talk about the base of a log, we're talking about 2 or
  1.  In the case of a BST case it's 2 (we'll dive into why that is shortly).

- Q: What does `n` represent?

  A: `n` _usually_ means "number of elements". In this case, `n` does mean that:
  or, another way to put that is "number of nodes in the tree".
